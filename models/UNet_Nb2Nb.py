...
class UNet_Nb2Nb(nn.Module):
    def __init__(self, c_in=1, c_out=1, residual=False, down='conv', up='tconv', activation='selu',
                 post_processing=False):
        super(UNet_Nb2Nb, self).__init__()
        ...
        self.conv1 = ConvBlock(c_in, 32, residual, activation)
        ...
        if self.residual:
            self.convres = ConvBlock(c_in, c_out, residual, activation)
        if self.post_process:
            self.conv1x1 = nn.Sequential(nn.Conv2d(64, 64, 1, 1), nn.LeakyReLU(),
                                         nn.Conv2d(64, 64, 1, 1), nn.LeakyReLU(),
                                         nn.Conv2d(64, 64, 1, 1), nn.LeakyReLU(),
                                         nn.Conv2d(64, c_out, 1, 1), nn.ReLU())
...
if __name__ == '__main__':
    ...
    import time
    net = UNet_Nb2Nb(3,3,post_processing=True).cuda()
    with torch.no_grad():
