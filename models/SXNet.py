...
class vanilla_bone(nn.Module):
    def __init__(self, act_name, asy, bias, basic_ch):
        super(vanilla_bone, self).__init__()
        self.conv_2 = conv_norm_act(basic_ch, basic_ch, k=3, pad=1, bias=bias, act_name=act_name, asy=asy)
        self.conv_3 = conv_norm_act(basic_ch, basic_ch, k=3, pad=1, bias=bias, act_name=act_name, asy=asy)
        self.conv_4 = conv_norm_act(basic_ch, basic_ch, k=3, pad=1, bias=bias, act_name=act_name, asy=asy)
        self.conv_5 = conv_norm_act(basic_ch, basic_ch, k=3, pad=1, bias=bias, act_name=act_name, asy=asy)
        self.conv_6 = conv_norm_act(basic_ch, basic_ch, k=3, pad=1, bias=bias, act_name=act_name, asy=asy)
        self.conv_7 = conv_norm_act(basic_ch, basic_ch, k=3, pad=1, bias=bias, act_name=act_name, asy=asy)
        self.conv_8 = conv_norm_act(basic_ch, basic_ch, k=3, pad=1, bias=bias, act_name=act_name, asy=asy)
        self.conv_9 = conv_norm_act(basic_ch, basic_ch, k=3, pad=1, bias=bias, act_name=act_name, asy=asy)
        self.conv_10 = conv_norm_act(basic_ch, basic_ch, k=3, pad=1, bias=bias, act_name=act_name, asy=asy)
        self.conv_11 = conv_norm_act(basic_ch, basic_ch, k=3, pad=1, bias=bias, act_name=act_name, asy=asy)
        self.conv_12 = conv_norm_act(basic_ch, basic_ch, k=3, pad=1, bias=bias, act_name=act_name, asy=asy)
        self.conv_13 = conv_norm_act(basic_ch, basic_ch, k=3, pad=1, bias=bias, act_name=act_name, asy=asy)
        self.conv_14 = conv_norm_act(basic_ch, basic_ch, k=3, pad=1, bias=bias, act_name=act_name, asy=asy)
        self.conv_15 = conv_norm_act(basic_ch, basic_ch, k=3, pad=1, bias=bias, act_name=act_name, asy=asy)
        self.conv_16 = conv_norm_act(basic_ch, basic_ch, k=3, pad=1, bias=bias, act_name=act_name, asy=asy)
...
class mkm_rm_bone(nn.Module):
    def __init__(self, act_name, asy, bias, basic_ch):
        super(mkm_rm_bone, self).__init__()
...
class DNCNN_based(nn.Module):
    def __init__(self, in_c, bone_name, phase):
        super(DNCNN_based, self).__init__()
        self.phase = phase
        if bone_name == 'mkm':
            basic_ch = 32
            self.bone = mkm_bone(act_name='mish', asy=True, bias=False, basic_ch=basic_ch)
        elif bone_name == 'rm':
            basic_ch = 32
            self.bone = rm_bone(act_name='mish', asy=True, bias=False, basic_ch=basic_ch)
        elif bone_name == 'mkm_rm':
            basic_ch = 32
            self.bone = mkm_rm_bone(act_name='mish', asy=True, bias=False, basic_ch=basic_ch)
        elif bone_name == 'vanilla':
            basic_ch = 64
            self.bone = vanilla_bone(act_name='relu', asy=False, bias=False, basic_ch=basic_ch)
        self.conv_in = nn.Conv2d(in_channels=in_c, out_channels=basic_ch, kernel_size=3, padding=1, bias=True)
        self.conv_out = nn.Conv2d(in_channels=basic_ch, out_channels=in_c, kernel_size=3, padding=1, bias=False)
    def forward(self, x):
        x_in = self.conv_in(x)
        x1 = self.bone(x_in)
        x1 = self.conv_out(x1)
...
class SXNet(nn.Module):
    def __init__(self, basic_ch, rfb_ch, in_c, phase, act_name='mish', asy=True, bias=False, tlu=False):
        super(SXNet, self).__init__()
        print('basic_ch:{}, rfb_ch:{}, in_c:{}, phase:{}, act_name:{}, asy:{}, bias:{}, tlu:{}'
              .format(basic_ch, rfb_ch, in_c, phase, act_name, asy, bias, tlu))
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        # net = SXNet(32, 32, 3, 'test', asy=True, tlu=True).cuda()
        net = DNCNN_based(3, 'vanilla', 'test').cuda()
